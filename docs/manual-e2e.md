...
**Terminal-query-response leak (OSC 11)** — Remote shell startup (oh-my-zsh + zsh-autosuggestions + prompt-init) emits OSC 11 (foreground-color) queries whose responses leak into zmx scrollback as stray `11;rgb:...` characters. The fix is laptop-side: the installer sets `osc-color-report-format = none` in the managed Ghostty config block, so Ghostty never responds to OSC 4/10/11 color queries and no response bytes are generated to leak. The server-side remote-env block also sets `ZSH_AUTOSUGGEST_HIGHLIGHT_STYLE='fg=8'` as defense-in-depth. **CSI 6n (cursor-position) responses** are a separate, lower-severity known limitation: Ghostty has no config to disable DSR cursor-position reporting, so a remote shell that emits CSI 6n will still see its response (`<r>;<c>R`) captured in scrollback. This is less visually disruptive than the OSC 11 leak and is tracked as a follow-up (requires either an upstream Ghostty DSR-disable option or a zmx-side input filter).
