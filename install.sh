...
# The early-source line runs the inherit hook from .zprofile BEFORE .zshrc,
# so split panes exec the projection wrapper before any .zshrc plugin emits
# OSC 11 / CSI 6n terminal queries (Prototype A).
...
print_plan() {
  ...
  print "  - update $zprofile with one guarded early-source line (Prototype A)"
...
# Prototype A: add the early-source line to .zprofile so the inherit hook
# runs before .zshrc (kills the OSC 11 / CSI 6n query leak on split panes).
